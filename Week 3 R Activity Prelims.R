...
dim(wdf) # number of rows, columns
nrow(wdf) # number of rows
ncol(wdf) # number of columns
colnames(wdf) # column names

str(wdf) # class of columns 
lapply(wdf, class) # class of columns 
...
wdf1 <- data.frame(weigth, heightt)
...
ggplot(wdf, aes(x=height, y=weight)) + 
  geom_point(shape = 17, colour = "green", fill = "green", size = 5, stroke = 1)
